...
from .perms import check_perms, check_authenticated
from .models import GuardedModel, GuardedModelManager


class CustomizeMutationErrorType(graphene.ObjectType):
    ...
    field = graphene.String(description=("The field that caused the error, or `null` if it " "isn't associated with any particular field."), required=False,)
...
class ModelType(_BaseDjangoObjectType):
    ...
    def get_queryset(cls, qs, info):
        ...
        if cls._meta.object_permissions and isinstance(cls._meta.model.objects, GuardedModelManager):
            qs &= cls._meta.model.objects.for_user(info.context.user, cls._meta.object_permissions, any_perm=cls._meta.object_permissions_any,)
        ...
        prl = {i.to_attr if isinstance(i, Prefetch) else i: i for i in ret._prefetch_related_lookups}
    ...
    def get_node(cls, info, id):
        ...
        if gql_optimizer is not None:
            ...
            try:
                instance = cls.get_queryset(cls._meta.model.objects, info).get(pk=id,)
