...
from .types import CustomizeMutationErrorType
from .perms import check_perms, check_authenticated, check_superuser
...
def _get_output_fields(model, return_field_name):
    ...
    if not model_type:  # pragma: no cover
        raise ImproperlyConfigured("Unable to find type for model {} in graphene registry".format(model.__name__,))
    f = graphene.Field(lambda: _registry.get_type_for_model(model), description="The mutated object.",)
    ...
    if hasattr(validation_error, "error_dict"):
        ...
        for field, field_errors in validation_error.message_dict.items():
            for e in field_errors:
                ...
                e_list.append(CustomizeMutationErrorType(field=field, message=e))
    else:
        # convert non-field errors
        for e in validation_error.error_list:
            e_list.append(CustomizeMutationErrorType(message=e.message))

    return e_list


def _get_fields(model, only_fields, exclude_fields, required_fields):
    fields = [(field.name, field) for field in sorted(list(model._meta.fields + model._meta.many_to_many))]
    ...
    fields.extend(
        [
            ...
            for field in sorted(list(model._meta.related_objects), key=lambda field: field.name,)
    ...
    for name, field in fields:
        if (only_fields and name not in only_fields) or name in exclude_fields or str(name).endswith("+") or name in ["created_at", "updated_at", "archived_at"]:
            continue

        if name == "id":
            ret[name] = graphene.ID(description="The ID of the object.",)
        elif isinstance(field, models.FileField):
            ret[name] = Upload(description=field.help_text,)
            ...
            ret[name] = graphene.List(graphene.ID, description=field.help_text,)
            ...
            ret[name] = graphene.List(graphene.ID, description="Set list of {0}".format(field.related_model._meta.verbose_name_plural,),)
        else:
...
def _is_id_field(field):
    return field.type == graphene.ID or isinstance(field.type, graphene.NonNull) and field.type.of_type == graphene.ID
...
def _get_model(field, model):
    for relation in model._meta.get_fields():
        if isinstance(relation, (models.ForeignKey, models.OneToOneField)) and relation.name == field:
            return relation.remote_field.model
    return model
...
class BaseMutationOptions(MutationOptions):
    ...
    # If we should allow super user to do this mutation
    permissions_superuser = False
...
class BaseMutation(ClientIDMutation):
    ...
    errors = graphene.List(graphene.NonNull(CustomizeMutationErrorType), description="List of errors that occurred while executing the mutation.",)

    # status of the mutation
    status = graphene.Boolean(default_value=False, description="Status of the mutation.",)

    @classmethod
    def __init_subclass_with_meta__(cls, permissions=None, permissions_any=True, allow_unauthenticated=False, permissions_superuser=False, _meta=None, **kwargs):
        if not _meta:
        ...
        _meta.permissions_superuser = permissions_superuser
    ...
    def get_node(cls, info, node_id, field="id", only_type=None):
        ...
        try:
            model = _get_model(field, cls._meta.model)
            node_type = _registry.get_type_for_model(model)._meta.name
            node = get_node(node_id, node_type, only_type)
    ...
    def get_nodes(cls, ids, field, only_type=None):
        ...
        try:
            model = _get_model(field, cls._meta.model)
            node_type = _registry.get_type_for_model(model)._meta.name
            instances = get_nodes(ids, node_type, only_type)
    ...
    def check_permissions(cls, user):
        ...
        if cls._meta.permissions_superuser and not check_superuser(user):
            return False
...
class BaseModelMutation(BaseMutation):
    ...
    def __init_subclass_with_meta__(
        ...
        **kwargs
        ...
        _meta.node = _registry.get_type_for_model(model)._meta.name

        super().__init_subclass_with_meta__(
            _meta=_meta, input_fields=input_fields, **kwargs,
        )
    ...
    def check_object_permissions(cls, user, instance):
        ...
        return instance.has_perm(user, cls._meta.object_permissions, any_perm=cls._meta.object_permissions_any,)
...
class ModelOperationMutation(BaseModelMutation):
    ...
    def __init_subclass_with_meta__(cls, **kwargs):
        super().__init_subclass_with_meta__(
            only_fields=["id"], required_fields=["id"], **kwargs,
        )
...
class ModelMutation(BaseModelMutation):
    ...
    def perform_mutation(cls, root, info, **data):
        ...
        for f in itertools.chain(instance._meta.many_to_many, instance._meta.related_objects, instance._meta.private_fields,):
        ...
        return cls(**{cls._meta.return_field_name: instance}, status=True)
...
class ModelUpdateMutation(ModelMutation):
    ...
    def __init_subclass_with_meta__(cls, **kwargs):
        ...
        super().__init_subclass_with_meta__(
            required_fields=required_fields, **kwargs,
        )
