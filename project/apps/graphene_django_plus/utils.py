...
def _resolve_nodes(ids, node_type, graphene_type=None):
    pks = []
    invalid_ids = []
...
def get_node(id, node_type, graphene_type=None):
    """Get a node given the relay id."""
    if not graphene_type:
        graphene_type = _resolve_graphene_type(node_type)

    return graphene_type._meta.model.objects.get(pk=id)


def get_nodes(ids, node_type, graphene_type=None):
    """Get a list of nodes.
    ...
    """
    nodes_type, pks = _resolve_nodes(ids, node_type, graphene_type)
